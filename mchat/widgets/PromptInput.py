...
class PromptInput(Widget):
    ...
    class Changed(Message):
        ...
        """The result of validating the value (formed by combining the results from
            each validator), or None if validation was not performed (for example when
            no validators are specified in the `Input`s init)"""
...
class MultiLineInput(Widget, can_focus=True):
    ...
    MultiLineInput>.input--cursor {
        background: $surface;
        color: $text;
        text-style: reverse;
    }
    MultiLineInput>.input--placeholder, Input>.input--suggestion {
        color: $text-disabled;
    }
    MultiLineInput.-invalid {
        border: tall $error 60%;
    }
    MultiLineInput.-invalid:focus {
        border: tall $error;
    }
    """
        ...
        *,
        ...
        Args:
            ...
            suggester: [`Suggester`][textual.suggester.Suggester] associated with this
                input instance.
            validators: An iterable of validators that the Input value will be checked
                against.
        ...
        # Below will be manipualted by the parent widget
        self._multiline_mode = False
        """ Flag to indicate if the input is in multiline mode or not. """

        self.add_class("singeline-input")
        """ set class to indicate the input type for styling """

        self.border_title = "Multi-Line: Off"
        """ Will toggle to "Multi-Line: On" when the input is in multiline mode. """
    ...
    async def _watch_value(self, value: str) -> None:
        ...
        self.post_message(PromptInput.Changed(self, value, validation_result))
    ...
    def validate(self, value: str) -> ValidationResult | None:
        ...
        Returns:
            A ValidationResult indicating whether *all* validators succeeded or not.
                ...
                validation.
        """
        # If no validators are supplied, and therefore no validation occurs, we return
        # None.
    ...
    async def _on_key(self, event: events.Key) -> None:
        ...
        # handle carriage return for multiline input
    ...
    async def action_submit(self) -> None:
        ...
        Normally triggered by the user pressing Enter. This will also run any
        validators."""
        self.log(f"Submitted: {self.value}")
        validation_result = self.validate(self.value)
        self.post_message(PromptInput.Submitted(self, self.value, validation_result))
